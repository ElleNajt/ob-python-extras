...
* Features:
:PROPERTIES:
:header-args: :results output drawer :async t :tangle :session project :timer-show no :exports both
:END:
...
- Print pandas [[file:tests/babel-formatting.org::*Displaying styled dataframes as pngs][dataframes as org tables by default]], by overwriting the __repr__ method. Uses tabulate if available. Index display can be toggled with ~print_org_df.set_show_index(False)~.
- Displayed styled dataframes as embedded pngs, using dataframe_image.
- Pretty print PyTorch tensors using rich library with syntax highlighting and indentation guides
...
|   5 | 6 | 15 |
|   6 | 7 | 16 |
[[file:plots/Readme/plot_20250309_231625_844246.png]]
...tada!
:end:

#+begin_src python :dpi 200 :dataframe_image yes :results drawer :exports both
print("that same dataframe as a styled dataframe")
styled_df = df.head(3).style.background_gradient()
print(styled_df)
#+end_src

#+RESULTS:
:results:
that same dataframe as a styled dataframe
[[file:plots/Readme/df_plot_20250310_001026_1904424.png]]
:end:
...
- Image inspector: Enlarge images with C-c C-c when over an image, quit with q.
...
** Gptel integrations:
This was moved into ElleNajt/gpt-babel to handle more source blocks than just python. This allows one to send cells and output into a conversations with an LLM that carries the same sessions, provides tools to make it easy to patch LLM corrections back onto original cells.
...
- System:  Pandoc (For conversions)
- Emacs: Eglot (for lsp integrations), Dired (For the conversion scripts)
...
#+end_src
 ...
 Whenever an org file is saved, or emacs is closed, a script will run that collects all the open org files, and checks the folders in /plots/{buffer_name} where the images are stored, and deletes those that are not referenced.
...
This is now at ElleNajt/org-babel-alert
...
Auto formats source blocks using black and isort. Can be turned on and off with:

#+begin_src elisp
(setq ob-python-extras/auto-format t)
#+end_src

This requires isort to be available on your path, and to configure python-black.
...
This package provides some overlapping functionality, and as far as I understand it does it by connecting to jupyter kernels and providing a front end in org mode for the kernel. I have not used this myself yet.
