...
Then, every 5 minutes, a script will run that collects all the open org files, and checks the folders in /plots/{buffer_name} where the images are stored, and deletes those that are not referenced.
*** TODO Modify the html conversion stuff to go in the same folder
